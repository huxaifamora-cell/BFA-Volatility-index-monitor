...
void ResyncAllSignals()
{
   ...
   for(int i = 0; i < ArraySize(activeSignals); i++)
   {
      ...
      string json = BuildSignalJSON(activeSignals[i].symbol, activeSignals[i].timeframe,
                                    ...
                                    activeSignals[i].minMargin, activeSignals[i].volLevel,
                                    activeSignals[i].priority,
...
string BuildSignalJSON(string symbol, string tf, string tradeType, string h4Trend, string d1Trend,
                       double minLot, double minMargin, string volLevel, int priority, datetime ts)
{
   int lotDigits = 2;
...
void CheckAndUpdateSignal(int symbolIndex, bool isH1)
{
   ...
   if(conditionsMet && !wasValid)
   {
      ...
      AddActiveSignal(symbolName, timeframe, tradeType, h4Trend, d1Trend, minLot, minMargin, volLevel);
      SendSignalToWebSocket(symbolName, timeframe, tradeType, h4Trend, d1Trend, minLot, minMargin, volLevel);
...
void AddActiveSignal(string symbolName, ENUM_TIMEFRAMES timeframe, string tradeType,
                     string h4Trend, string d1Trend, double minLot, double minMargin, string volLevel)
{
                           ...
                           string h4Trend, string d1Trend, double minLot, double minMargin, string volLevel)
{
   ...
   string json = BuildSignalJSON(displaySymbol, tf, tradeType, h4Trend, d1Trend,
                                 minLot, minMargin, volLevel, priority, TimeCurrent());
