...
void ResyncAllSignals()
{
   ...
   for(int i = 0; i < ArraySize(activeSignals); i++)
   {
      ...
      string json = BuildSignalJSON(activeSignals[i].symbol, activeSignals[i].timeframe,
                                    ...
                                    activeSignals[i].minMargin, activeSignals[i].priority,
...
string BuildSignalJSON(string symbol, string tf, string tradeType, string h4Trend, string d1Trend,
                       double minLot, double minMargin, int priority, datetime ts)
{
   // Use enough decimal places for small lot sizes like 0.001
   int lotDigits = 2;
...
//+------------------------------------------------------------------+
// For Volatility indices: signal logic is DIRECTION-AGNOSTIC
// BUY  signal: BB slope up   + EMA above BB middle + EMA above SMA
// SELL signal: BB slope down + EMA below BB middle + EMA below SMA
//+------------------------------------------------------------------+
                           ...
                           string h4Trend, string d1Trend, double minLot, double minMargin)
{
   ...
   string json = BuildSignalJSON(displaySymbol, tf, tradeType, h4Trend, d1Trend,
                                 minLot, minMargin, priority, TimeCurrent());
